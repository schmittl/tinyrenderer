(function (scope) {
  ...
  function TinyRenderer(canvas) {
    this.canvas = canvas;
    this.ctx = canvas.getContext("2d");
    this.zbuffer = new Float32Array(canvas.width * canvas.height);
    this.zbuffer.fill(-32768);
    this.zdepth = 2000;
    this.shadowBuffer = new Float32Array(canvas.width * canvas.height);
    this.shadowBuffer.fill(-32768);
    this.viewport = this.getViewport(mat4.create(), canvas.width / 8, canvas.width / 8, canvas.width * 3 / 4 , canvas.height * 3 / 4);
  }
  ...
  TinyRenderer.prototype.getProjection = function(out, coefficient) {
    out[0]  = 1;
    out[5]  = -1; // flip y axis
    out[10] = 1;
    out[11] = coefficient; // coefficient for perspective division
    return out;
  };

  TinyRenderer.prototype.triangle = function(clipCoords, shader, imageData, zbuffer) {
    var view0 = clipCoords[0], view1 = clipCoords[1], view2 = clipCoords[2]; // homogeneous
    ...
    for (var x = minX; x <= maxX; x++) {
      for (var y = minY; y <= maxY; y++) {
        ...
        w =  clipCoords[0][3] * BC[0] + clipCoords[1][3] * BC[1] + clipCoords[2][3] * BC[2];
        fragDepth = Math.trunc(z / w);
        zOffset = x + y * this.canvas.width;

        if (fragDepth >= zbuffer[zOffset]) {
          if (shader.fragment(BC, color)) {
            continue;
          }
          zbuffer[zOffset] = fragDepth;
  ...
  TinyRenderer.prototype.renderModel = function(model, shader, imageData, buffer) {
    ...
    for (var i = 0; i < length; i += 3) {
      ...
      this.triangle(clipCoords, shader, imageData, buffer);
    }
  };
